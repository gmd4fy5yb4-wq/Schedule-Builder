...
- **Trial clock NOT yet enforced:** `trial_started_at` is written but no middleware reads it — the 14-day trial does not auto-expire yet (deferred with the trial-trigger fix).

## Known Issues / Do-Not-Touch
- ⚠️ **Trial trigger missing in prod.** The `handle_new_user` / `on_auth_user_created` trigger from migration 002 does NOT exist in the Sports DB, so new signups get no `user_subscriptions` row and are gated straight to `/pricing` (no free trial). Fix deferred — re-create the trigger + backfill rowless users (`ON CONFLICT DO NOTHING`). Pair with enforcing the 14-day trial clock (`trial_started_at`) in middleware.
